...
local aA=240
local aB=false
local aC=ae("DISCORD","key",nil,"Secondary",ax.Frame)
...
if ag.Hover then
    ...
    aa.AddSignal(aC.MouseEnter, function()
        if aj then
            ...
            mouseMoveConnection = aa.AddSignal(aC.MouseMoved, function(aE, aF)
    ...
    aa.AddSignal(aC.MouseLeave, function()
        if aj then
...
au.IsMinimized = false
au.OriginalSize = nil

function au.Minimize()
    if au.IsMinimized then
        au.UIElements.MainBar.Visible = true
        au.UIElements.SideBarContainer.Visible = true
        am(au.UIElements.Main, 0.35, {Size = au.OriginalSize}, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
        au.IsMinimized = false
    else
        au.OriginalSize = au.UIElements.Main.Size
        au.UIElements.MainBar.Visible = false
        au.UIElements.SideBarContainer.Visible = false
        am(au.UIElements.Main, 0.35, {Size = UDim2.new(au.OriginalSize.X.Scale, au.OriginalSize.X.Offset, 0, au.Topbar.Height)}, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
        au.IsMinimized = true
    end
end
